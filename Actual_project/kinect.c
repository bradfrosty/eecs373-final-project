...
void init_kinect() {
	...
	MSS_UART_set_rx_handler(&g_mss_uart1, kinect_data_handler, MSS_UART_FIFO_SINGLE_BYTE);
}
...
void process_camera_data(uint8_t rx_buff, uint32_t rx_size) {
	...
	if ((rx_buff & 0xF0) == 0x00 && (rx_buff & 0x0F) != dir_y) {
		MYMOTORS->direction_y = rx_buff & 0x0F;
		dir_y = (rx_buff & 0x0F);
	}
	else if ((rx_buff & 0xF0) == 0x10 && (rx_buff & 0x0F) != dir_x) {
